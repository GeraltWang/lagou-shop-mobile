<template>
  <van-cell-group v-if="networkStatus" :border="false" class="comment">
    <!-- 评价概况 -->
    <van-cell title="商品评价" :value="`好评率：${state.replyChance}%`" />
    <!-- 评价切换 -->
    <van-cell class="tags">
      <van-button size="small" :class="{ active: state.isSum }" @click="onChange(0)">全部({{ state.sumCount }})</van-button>
      <van-button size="small" :class="{ active: state.isGood }" @click="onChange(1)">好评({{ state.goodCount }})</van-button>
      <van-button size="small" :class="{ active: state.isIn }" @click="onChange(2)">中评({{ state.inCount }})</van-button>
      <van-button size="small" :class="{ active: state.isPoor }" @click="onChange(3)">差评({{ state.poorCount }})</van-button>
    </van-cell>
    <!-- 评价列表 -->
  ...
  <van-empty v-else image="network" description="服务器走丢了">
    <van-button round type="danger">刷新试试</van-button>
  </van-empty>
</template>
